...
def analyze_pair(on_id, off_id, tmscore, filter_failed_docking=True):
    ...
    models = {
        ...
        'Reference': results_dir / 'Reference_tmscore_evaluation' / f'id{on_id}_high_{off_id}' / 'docking_results.json',
        ...
        'KGDiff': Path('./results/unified_kgdiff_retrain') / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        ...
        'dual_1p_w2_1': results_dir / 'redock_dual_head2_1p_all_w2_1' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'dual_atom_w2_1': results_dir / 'redock_dual_head2_atom_w2_1' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        # === NEW: Unified model results ===
        'Unified_H1H2_BiNG': Path('./results/unified_h1h2_bi_noguide') / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'Unified_H1_BiOn': Path('./results/unified_h1_bi_ori') / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'Unified_H2_BiOff': Path('./results/unified_h2_bi_offguide') / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        # === NEW: Unified model variants ===
        'Unified_ProType': results_dir / 'unified_h1h2_pro_type' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'Unified_ProPos': results_dir / 'unified_h1h2_pro_pos' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'Unified_LigType': results_dir / 'unified_h1h2_lig_type' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'Unified_LigPos': results_dir / 'unified_h1h2_lig_pos' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'Unified_BiType': results_dir / 'unified_h1h2_bi_type' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
        'Unified_BiPos': results_dir / 'unified_h1h2_bi_pos' / f'id{on_id}_{off_id}_high' / 'docking_results' / 'docking_results.json',
...
def print_detailed_report(all_pairs_results, final_stats, filter_failed_docking=True):
    ...
    if filter_failed_docking:
        print("DETAILED DOCKING RESULTS ANALYSIS FOR HIGH TM-SCORE PAIRS (FILTERED)")
        print("=" * 160)
        print("\n*** DOCKING FAILURE FILTER APPLIED: Molecules with on-target OR off-target score >= 0 are EXCLUDED ***\n")
    else:
        print("DETAILED DOCKING RESULTS ANALYSIS FOR HIGH TM-SCORE PAIRS (UNFILTERED)")
        print("=" * 160)
...
def main():
    parser = argparse.ArgumentParser(description='Analyze docking results for HIGH TM-score pairs')
    ...
    # Save to file
    if filter_failed_docking:
        output_file = 'tmscore_pairs_analysis_high_filtered.txt'
    else:
        output_file = 'tmscore_pairs_analysis_high_unfiltered.txt'
